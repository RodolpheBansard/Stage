...
namespace InfinityCode.RealWorldTerrain
{
    public static class RealWorldTerrainGUIUtils
    {
        ...
        private static readonly string[] labelsTextureSize = {"32", "64", "128", "256", "512", "1024", "2048", "4096", "8192"};
        private static readonly int[] valuesTextureSize = { 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192 };
...
#region General
        ...
        private static void DrawDecimalCoordinates()
        {
        ...
        private static void DrawUTMCoordinates()
        {
            GUILayout.Label("Top-Left");
            EditorGUI.indentLevel++;
        ...
        private static void OnIdleGUI()
        {
            ...
            if (isFull)
            {
                ...
                if (showCoordinates)
                {
                    ...
                    if (EditorGUI.EndChangeCheck() && coordinateMode == 1)
                    {
                        double e, n;
                    ...
                    if (coordinateMode == 0) DrawDecimalCoordinates();
                    else DrawUTMCoordinates();
...
#region Texture
        ...
        private static bool DrawVectorLayerData(RealWorldTerrainVectorTerrainLayerFeature.Rule data, int index)
        {
            bool ret = false;

            EditorGUILayout.BeginHorizontal();
            GUILayout.Label("Rule " + (index + 1) + ": ");
            if (GUILayout.Button("X", GUILayout.ExpandWidth(false))) ret = true;
            EditorGUILayout.EndHorizontal();

            EditorGUI.BeginChangeCheck();
            data.layer = (RealWorldTerrainMapboxLayer)EditorGUILayout.EnumPopup("Layer", data.layer);
            if (EditorGUI.EndChangeCheck()) data.extra = ~0;
            if (data.hasExtra)
            {
                if (data.layer == RealWorldTerrainMapboxLayer.landuse)
                {
                    data.extra = (int)(RealWorldTerrainMapboxLanduse)EditorGUILayout.EnumFlagsField("Classes", (RealWorldTerrainMapboxLanduse) data.extra);
                }
                else if (data.layer == RealWorldTerrainMapboxLayer.landuse_overlay)
                {
                    data.extra = (int)(RealWorldTerrainMapboxLanduseOverlay)EditorGUILayout.EnumFlagsField("Classes", (RealWorldTerrainMapboxLanduseOverlay)data.extra);
                }
                else if (data.layer == RealWorldTerrainMapboxLayer.waterway)
                {
                    data.extra = (int)(RealWorldTerrainMapboxWaterway)EditorGUILayout.EnumFlagsField("Classes", (RealWorldTerrainMapboxWaterway)data.extra);
                }
            }
        ...
        private static bool DrawVectorTerrainLayer(RealWorldTerrainVectorTerrainLayerFeature layer, int index)
        {
            bool ret = false;
            EditorGUILayout.BeginHorizontal();
            GUILayout.Label("Layer " + (index + 1) + ": ");
            if (GUILayout.Button("X", GUILayout.ExpandWidth(false))) ret = true;
            EditorGUILayout.EndHorizontal();

            layer.terrainLayer = EditorGUILayout.ObjectField(new GUIContent("Terrain Layer"), layer.terrainLayer, typeof(TerrainLayer), false) as TerrainLayer;
            if (layer.rules == null) layer.rules = new List<RealWorldTerrainVectorTerrainLayerFeature.Rule>();

            int removeIndex = -1;
            for (int i = 0; i < layer.rules.Count; i++)
            {
                if (DrawVectorLayerData(layer.rules[i], i)) removeIndex = i;
            }

            if (removeIndex != -1) layer.rules.RemoveAt(removeIndex);
            if (GUILayout.Button("Add Rule")) layer.rules.Add(new RealWorldTerrainVectorTerrainLayerFeature.Rule());
        ...
        private static void RegularTextureGUI()
        {
            TextureSourceGUI();
            ...
            prefs.textureMipMaps = EditorGUILayout.Toggle("Mipmaps", prefs.textureMipMaps);
...
#if UNITY_2018_3_OR_NEWER
        private static void TerrainLayersGUI()
        {
            ...
            MapboxAccessToken();
            TextureMaxLevelGUI();
            prefs.vectorTerrainBaseLayer = EditorGUILayout.ObjectField(new GUIContent("Base Layer"), prefs.vectorTerrainBaseLayer, typeof(TerrainLayer), false) as TerrainLayer;

            if (prefs.vectorTerrainLayers == null) prefs.vectorTerrainLayers = new List<RealWorldTerrainVectorTerrainLayerFeature>();
            int removeIndex = -1;

            for (int i = 0; i < prefs.vectorTerrainLayers.Count; i++)
            {
                if (DrawVectorTerrainLayer(prefs.vectorTerrainLayers[i], i))
                {
                    removeIndex = i;
                }
            }

            if (removeIndex != -1) prefs.vectorTerrainLayers.RemoveAt(removeIndex);

            if (GUILayout.Button("Add Layer")) prefs.vectorTerrainLayers.Add(new RealWorldTerrainVectorTerrainLayerFeature());
            GUILayout.Space(10);
        }
#endif
        ...
        {
            prefs.textureResultType = (RealWorldTerrainTextureResultType)EditorGUILayout.EnumPopup("Result Type", prefs.textureResultType);

            if (prefs.textureResultType == RealWorldTerrainTextureResultType.regularTexture) RegularTextureGUI();
            else if (prefs.textureResultType == RealWorldTerrainTextureResultType.hugeTexture) HugeTextureGUI();
#if UNITY_2018_3_OR_NEWER
            else if (prefs.textureResultType == RealWorldTerrainTextureResultType.terrainLayers) TerrainLayersGUI();
#endif

            EditorGUILayout.Space();
        }

        private static void TextureMaxLevelGUI()
        {
        ...
        private static void TextureSourceGUI()
        {
            TextureProviderGUI();
...
#region Other
        ...
        private static void OSMBuildings()
        {
            ...
            if (prefs.generateBuildings)
            {
...
#endif
#if BUILDR3
                availableBuilders.Add("BuildR3");
                availableBuildersID.Add(5);
#endif
#if PROCEDURAL_TOOLKIT
                ...
                else if (prefs.buildingGenerator == 5)
                {
#if BUILDR3
                    //prefs.buildRCollider = (RealWorldTerrainBuildR2Collider)EditorGUILayout.EnumPopup("Collider", prefs.buildRCollider);
                    //prefs.buildRRenderMode = (RealWorldTerrainBuildR2RenderMode)EditorGUILayout.EnumPopup("Render Mode", prefs.buildRRenderMode);
